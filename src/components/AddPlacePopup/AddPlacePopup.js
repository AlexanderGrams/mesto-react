  ...
  return (
    <PopupWithForm onSubmit={handleAddPlaceSubmit} name={'add-card'} title={'Новое место'} isOpen={isOpen} onClose={onClose} buttonText="Сохранить">
      <fieldset className="popup__editing-profille">
        <input value={nameCard} id="title-input" className="popup__item popup__item_type_title" type="text" name="name" required minLength="2" maxLength="30" placeholder="Название" onChange={(e)=> {handleChange(e, setNameCard)}} />
        <span id="title-input-error" className="popup__text-error"></span>
        <input value={linkCard} id="link-input" className="popup__item popup__item_type_link" type="url" name="link" required placeholder="Ссылка на картинку" onChange={(e)=> {handleChange(e, setLinkCard)}} />
